...
function showProduct() {
    ...
    if(selectOp == "Tất cả") {
        result = product.filter((item) => item.status == 1);
    } else if(selectOp == "Đã xóa") {
        result = product.filter((item) => item.status == 0);
    } else {
        result = product.filter((item) => item.category == selectOp);
    }
...
function cancelSearchProduct() {
    let product = localStorage.getItem("product") ? JSON.parse(localStorage.getItem("product")).filter(item => item.status == 1) : [];
    document.getElementById('the-loai').value = "Tất cả";
    document.getElementById('form-search-product').value = "";
    displayList(product, perPage, currentPage);
    setupPagination(product, perPage, currentPage);
}
...
function editProduct(id) {
    ...
    //
    document.querySelector(".upload-image-preview").src = product[index].img;
    document.getElementById("ten-mon").value = product[index].title;
    document.getElementById("gia-moi").value = product[index].price;
    document.getElementById("mo-ta").value = product[index].desc;
    document.getElementById("chon-mon").value = product[index].category;
}
...
btnUpdateProductIn.addEventListener("click", (e) => {
    e.preventDefault();
    let product = JSON.parse(localStorage.getItem("product"));
    let idProduct = product[indexCur].id;
    let imgProduct = product[indexCur].img;
    let titleProduct = product[indexCur].title;
    let curProduct = product[indexCur].price;
    let descProduct = product[indexCur].desc;
    let categoryProduct = product[indexCur].category;
    ...
    if (imgProductCur != imgProduct || titleProductCur != titleProduct || curProductCur != curProduct || descProductCur != descProduct || categoryText != categoryProduct) {
        ...
        product.splice(indexCur, 1);
        product.splice(indexCur, 0, productadd);
        localStorage.setItem("product", JSON.stringify(product));
...
function detailOrder(id) {
    ...
    let orders = localStorage.getItem("order") ? JSON.parse(localStorage.getItem("order")) : [];
    let products = localStorage.getItem("order") ? JSON.parse(localStorage.getItem("product")) : [];
    // Lấy hóa đơn 
